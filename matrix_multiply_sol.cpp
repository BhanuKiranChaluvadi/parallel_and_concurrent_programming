/**
 * 
 * Now, that can turn into a lot of tasks (each element separate computation
 * in resultant matrix), especially for a large result matrix, 
 * so we will agglomerate those tasks based on row and will modify the 
 * number of rows in each group at run time, based on the number of processors 
 * that are available in the computer.
 * 
 * If the system only has two processors, then we'll combine the computations 
 * into two tasks. If it has four processors, then that's four tasks 
 * and so on. Our matrix multiplication solution has two parts. 
 *
 */
...
int main()
{
    ...
    // initialize A and B with values in range of 1 to 100
    ...
    for (size_t i = 0; i < NUM_ROWS_A; i++)
    {
        sequential_result[i] = (long *)malloc(NUM_COLS_A * sizeof(long));
    ...
    sequential_matrix_multiply(A, NUM_COLS_A, NUM_COLS_B,
                               B, NUM_ROWS_B, NUM_COLS_B,
                               sequential_result); // warm up
    ...
    parallel_matrix_multiply(A, NUM_COLS_A, NUM_COLS_B,
                             B, NUM_ROWS_B, NUM_COLS_B,
